...
PyQt6 기반 **실시간 근전도(EMG) 데이터 시각화** 대시보드입니다. 시리얼로 수신한 RAW/AMP를 그래프와 막대로 실시간 모니터링합니다. **4채널/6채널** 모드를 선택할 수 있습니다.
...
- **텍스트 (구현됨)**: 한 줄에 채널 수(N_CH)만큼의 실수(공백 구분), `\n` 종료.
  - 4ch: `123 456 789 012`
  - 6ch: `100 200 300 400 500 600`
- **프레임 (23 bytes, 문서화만)**: 헤더 2 + 데이터 20 + 체크섬 1 (XOR). 별도 파서 미구현.
...
- **아키텍처, 핵심 로직, UI·알고리즘 상세** → [PROJECT_DOCUMENTATION.md](PROJECT_DOCUMENTATION.md)
